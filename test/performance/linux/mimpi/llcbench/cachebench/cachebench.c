/* $Header: /shag/homes/mucci/.cvsrepository/cachebench/cachebench.c,v 1.4 1998/03/31 05:52:14 mucci Exp $ */
/* $Log: cachebench.c,v $
...
void do_memory_copy(int *sizes, void *x, double *times, double *bws, double *percents)
{
  ...
  double refcnt, nullcnt = 0.0, overhead_per_ref = 0.0, time_per_ref, cmicrosec = 0.0, tmicrosec;
  void *y;
  ...
  for (i = 0; i < timeslots; i++) 
      ...
      for (j = 0; j < repeat_count; j++)
	{
	  ...
	  time_per_ref = (tmicrosec*1000.0) / refcnt;
	  DBG(fprintf(stderr,"T: %f ns per ref.\n",time_per_ref));
	  ...
	  /* if (type & NOCALIBRATE)
	  ...
	  else
	    {
	      ...
	      if (time_per_ref < overhead_per_ref)
		{ 
		  fprintf(stderr,"Error: please recompile with less optimization.\n");
		  exit(1); 
		}
	    } */
